...
export async function POST(request: Request) {
  ...
  const { date, exerciseSlug, setNumber, completed } = body as { date: string; exerciseSlug: string; setNumber: number; completed: boolean };
  ...
  if (!exerciseId) return NextResponse.json({ error: "exercise_not_found", slug: exerciseSlug }, { status: 404 });

  const perf = await db.setPerformance.upsert({
    where: { sessionId_exerciseId_setNumber: { sessionId: session.id, exerciseId, setNumber } },
    create: { sessionId: session.id, exerciseId, setNumber, completed, reps: 0, weightKg: 0 },
    update: { completed },
  });
